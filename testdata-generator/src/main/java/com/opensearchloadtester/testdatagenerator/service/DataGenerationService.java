...
public class DataGenerationService {
      ...
      try {
        List<Document> documents =
            dataGenerator.generateData(
                dataGenerationProperties.getDocumentType(), currentBatchSize);
  ...
  private void generatePersistent(String indexName) {
    ...
    List<Document> documents =
        dataGenerator.generateData(
            dataGenerationProperties.getDocumentType(), dataGenerationProperties.getCount());
  ...
  /**
   * Handles retryable OpenSearch errors (413 / 429). Returns the adjusted batch size or throws the
   * exception if not retryable.
   */
    ...
    if (cause instanceof ResponseException responseException) {
      ...
      if (status == HTTP_PAYLOAD_TOO_LARGE || status == HTTP_TOO_MANY_REQUESTS) {
        ...
        if (newBatchSize == currentBatchSize) {
          log.error(
              "Batch size cannot be reduced further. Already at minimum ({}).", currentBatchSize);
          throw e;
        }

        log.warn(
            "OpenSearch returned HTTP {}. Reducing batch size from {} to {}",
            status,
            currentBatchSize,
            newBatchSize);
