...
/**
 * Generates and stores data persistently. On subsequent runs, existing data is reused unless
 * deleted manually.
 */
@Slf4j
...
public class PersistentDataGenerator implements DataGenerator {
  ...
  /**
   * Loads existing data from persistent file. If there is no data available, new random data is
   * generated
   */
  @Override
  public List<Document> generateData(DocumentType documentType, int count) {
    List<Document> existingData =
        switch (documentType) {
