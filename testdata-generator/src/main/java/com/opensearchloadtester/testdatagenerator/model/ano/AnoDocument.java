...
@Setter
@NoArgsConstructor(
    access = AccessLevel.PRIVATE) // AnoDocument objects must be created via random() method
public class AnoDocument extends AbstractDocument {

  /**
   * Attributes of an AnoDocument correspond to the data stored in an OpenSearch index. Wrapper
  ...
  public static AnoDocument random() {
    ...
    anoDocument.dssDeleteRetentionMinRetention =
    ...
    anoDocument.dssDocumentPath =
        "/" + anoDocument.dssDataspaceId + "/documents/" + anoDocument.dssDocumentId + "/document";
    anoDocument.dssDocumentSource = faker.internet().url();
    anoDocument.dssLastModifiedUserDatetime =
        Date.from(faker.timeAndDate().past(90, TimeUnit.DAYS));
    anoDocument.dssOriginalFilename =
        "Brutto-Netto-Abrechnung "
            + translateMonthToName(anoDocument.dssCustomMetadataPayrollInfo.getAccountingMonth())
            + " "
            + anoDocument.dssCustomMetadataPayrollInfo.getAccountingYear()
            + ".pdf";
    anoDocument.dssRecyclebin = false;

    anoDocument.dssDocumentName =
        "Brutto-Netto-Abrechnung "
            + translateMonthToName(anoDocument.dssCustomMetadataPayrollInfo.getAccountingMonth())
            + " "
            + anoDocument.dssCustomMetadataPayrollInfo.getAccountingYear()
    ...
    anoDocument.customAll =
        anoDocument.dssOriginalFilename + " " + anoDocument.dssProcessingFlagOwner;
