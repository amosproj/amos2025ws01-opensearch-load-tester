...
- `query_results.csv` – tabular view with raw JSON response in the last column.
...
2. `ReportService` converts each metric to `QueryResult`, appends to CSV and NDJSON, and updates in-memory stats only.
