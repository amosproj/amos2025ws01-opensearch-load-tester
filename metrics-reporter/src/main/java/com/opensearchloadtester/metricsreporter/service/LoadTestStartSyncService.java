...
public class LoadTestStartSyncService {
  ...
  public LoadTestStartSyncService(@Value("${load.generator.replicas}") int expectedLoadGenerators) {
  ...
  public synchronized void markReady(String loadGeneratorId) {
    ...
    log.info(
        "Load Generator with id '{}' marked as READY ({}/{})",
        loadGeneratorId,
    ...
    if (readyLoadGenerators.size() == expectedLoadGenerators && plannedStartTimeMillis == null) {
      ...
      log.info(
          "All Load Generators are ready. Planned load test start in {} seconds at {}",
  ...
  public synchronized LoadTestStartSyncStatusDto getStatus() {
    ...
    return new LoadTestStartSyncStatusDto(
        expectedLoadGenerators, readyLoadGenerators.size(), isStartAllowed, plannedStartTimeMillis);
  }
}
