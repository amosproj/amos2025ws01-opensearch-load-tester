...
class ReportControllerTest {
    ...
    /**
     * Tests the controller's null-check for the metrics list.
     * Note: In production, a missing/empty HTTP body would cause HttpMessageNotReadableException
     * before reaching the controller. However, a JSON body containing literal "null" would
     * be deserialized as null and reach this code path.
     */
    @Test
    ...
    void finish_setsExitCodeForFailedLoadGenerator() throws Exception {
        ...
        reportController.finish(
                new FinishLoadTestDto(LOAD_GENERATOR_ID, false, "Connection timeout"),
                request);
    ...
    void finish_isIdempotent_forSameLoadGenerator() throws Exception {
        ...
        // First finish call
        ResponseEntity<String> firstResponse = reportController.finish(
                new FinishLoadTestDto(LOAD_GENERATOR_ID, true, null),
                request);

        // Second finish call (idempotent)
        ResponseEntity<String> secondResponse = reportController.finish(
                new FinishLoadTestDto(LOAD_GENERATOR_ID, true, null),
                request);
