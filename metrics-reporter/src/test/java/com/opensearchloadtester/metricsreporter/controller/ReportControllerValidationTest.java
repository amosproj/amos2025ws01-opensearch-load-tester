 ...
 *
 * <p>These tests validate that the DTO validation annotations are correctly defined. The actual
 * enforcement via Spring MVC (@Valid on controller parameters) is tested implicitly when the
 * application runs, as the @ExceptionHandler in ReportController handles validation errors.
 *
 * <p>Note: MockMvc-based integration tests for @Valid on List<@Valid T> parameters have limitations
 * in the current Spring Boot test slice configuration. Full HTTP-stack validation is covered in
 * ReportControllerHttpValidationTest.
 */
class ReportControllerValidationTest {
  ...
  void metricsDto_isInvalid_forNegativeRequestDuration() {
    ...
    assertThat(violations)
        .anyMatch(v -> v.getPropertyPath().toString().equals("requestDurationMillis"));
  }
  ...
  void metricsDto_isInvalid_forNegativeQueryDuration() {
    ...
    assertThat(violations).isNotEmpty();
    assertThat(violations)
        .anyMatch(v -> v.getPropertyPath().toString().equals("queryDurationMillis"));
  }
