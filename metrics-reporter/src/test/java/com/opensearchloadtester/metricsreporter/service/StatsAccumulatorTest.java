...
class StatsAccumulatorTest {
  ...
  void update_handlesNullQueryDuration() {
    ...
    accumulator.update(List.of(new MetricsDto("lg-1", "match_all", 100L, null, 10, 200)));

    assertThat(accumulator.getTotalQueries()).isEqualTo(1L);
    assertThat(accumulator.getRequestDurationCount())
        .isEqualTo(1L); // primitive long, always counted
