...
public class LoadRunner {
  ...
  public void executeScenario(ScenarioConfig scenarioConfig) {
    log.info(
        "Executing '{}' (expected duration: {} sec)",
    ...
    try {
      ...
      // After the scenario duration, stop the periodic scheduling (already submitted tasks may
      // still run).
      scheduler.schedule(
          () -> {
      ...
      try {
        ...
        schedulerStopped =
            scheduler.awaitTermination(
                durationNs + TimeUnit.SECONDS.toNanos(10), TimeUnit.NANOSECONDS);
        if (!schedulerStopped) {
      ...
      if (workersStopped) {
        ...
        log.info(
            "Scenario '{}' completed successfully. All threads finished.",
            scenarioConfig.getName());
        log.info(
            "Schedule duration: {}s, Total duration: {}s",
            scenarioConfig.getScheduleDuration().getSeconds(),
            String.format("%.2f", actualDurationSeconds));
      } else {
        log.warn(
            "Scenario '{}' was interrupted while waiting for worker threads to finish. Actual runtime: {}s",
            scenarioConfig.getName(),
            String.format("%.2f", actualDurationSeconds));
      }
    } catch (Exception e) {
