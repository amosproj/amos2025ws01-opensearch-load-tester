 ...
 *
 * <p>Registers the Load Generator as READY and polls the shared synchronization status from the
 * Metrics Reporter until all Load Generators are allowed to start simultaneously or a timeout is
 * reached.
 */
@Slf4j
@Component
    ...
    while (true) {
      ...
      if (status.isStartAllowed()) {
        ...
        if (plannedStart < now) {
          throw new LoadTestStartSyncException(
              String.format(
                  "Missed global start (planned: %s, now: %s)",
          ...
          response -> {
            ...
            try {
              return objectMapper.readValue(
                  EntityUtils.toString(response.getEntity()), LoadTestStartSyncStatusDto.class);
