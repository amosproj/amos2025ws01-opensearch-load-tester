...
public class TestScenarioInitializer implements CommandLineRunner {
  ...
  private void runWarmUp() {
    log.info(
        "Running warm-up until at least {} requests executed AND at least {} ms elapsed",
        WARMUP_REQUEST_COUNT,
        MIN_WARMUP_DURATION_MS);
    ...
    if (!atLeastOneSuccessful) {
      throw new IllegalStateException(
          String.format(
              "Warm-up failed: 0 successful requests out of %s (%s ms total)",
              totalRequests, elapsedMs));
    }

    log.info("Warm-up completed in {} ms ({} total requests)", elapsedMs, totalRequests);
  }
