...
public class DuoInvoiceDisMaxExpensiveQuery extends AbstractQuery {
  ...
  private DuoInvoiceDisMaxExpensiveQuery(
  ...
  public static DuoInvoiceDisMaxExpensiveQuery random() {
    String invoiceNumber =
        String.format(
            "%d/%d",
            faker().number().numberBetween(10000, 99999), faker().number().numberBetween(1, 9999));

    String businessPartner = faker().company().name();

    List<String> termOptions =
        List.of("Wolle", "Leder", "Stahl", "Kupfer", "Marmor", "Gummi", "Papier", "Service");
