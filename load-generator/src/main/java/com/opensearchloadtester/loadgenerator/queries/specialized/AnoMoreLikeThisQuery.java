...
public class AnoMoreLikeThisQuery extends AbstractQuery {
  ...
  public static AnoMoreLikeThisQuery random() {
    ...
    String year = String.valueOf(faker().number().numberBetween(currentYear - 10, currentYear + 1));

    Map<String, String> queryParams = Map.of("year", year);

    return new AnoMoreLikeThisQuery(
        queryParams, "query-templates/specialized/ano_more_like_this.json");
  }
}
