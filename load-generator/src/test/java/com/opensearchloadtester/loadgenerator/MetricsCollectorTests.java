...
class MetricsCollectorTests {
  ...
  /**
   * Verifies that calling {@link MetricsCollector#appendMetrics(MetricsDto)} successfully sends the
   * metrics to the reporter client <br>
   * <br>
   * The test checks that:
   *
   * <ul>
   *   <li>The client receives exactly one batch
   *   <li>The batch contains the exact same instance that was provided
   * </ul>
   */
  @Test
  ...
  /**
   * Ensures that multiple sequential calls to {@link MetricsCollector#appendMetrics(MetricsDto)}
   * correctly send all provided metrics objects to the reporter client <br>
   * <br>
   * The test checks that:
   *
   * <ul>
   *   <li>The number of sent batches matches the number of inserted objects (batch size = 1)
   *   <li>Each inserted {@link MetricsDto} is sent to the client
   * </ul>
   */
  @Test
  ...
  /**
   * Verifies that {@link MetricsCollector#appendMetrics(MetricsDto)} is thread-safe due to its
   * synchronized buffer access <br>
   * <br>
   * The test launches multiple concurrent threads, each adding a metrics object. After all threads
   * finish, the client must have received exactly as many metrics as the number of executed threads
   * <br>
   * <br>
  ...
  void testAppendMetrics_threadSafety() throws InterruptedException {
    ...
    int totalMetricsSent = captor.getAllValues().stream().mapToInt(List::size).sum();
    assertEquals(threadCount, totalMetricsSent);
  }

  /**
   * Verifies that metrics are batched correctly when batch size is greater than 1 <br>
   * <br>
   * The test checks that:
   *
   * <ul>
   *   <li>No metrics are sent until the batch size is reached
   *   <li>When batch size is reached, all metrics are sent together
   * </ul>
   */
  @Test
